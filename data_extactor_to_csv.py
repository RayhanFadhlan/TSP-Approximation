...
def main(num_countries):
    ...
    ratio_mst = cost_mst/cost_dp
    ratio_alter = alter_cost/cost_dp



    print("Ratio : " , alter_cost/cost_dp)
    return num_countries, cost_mst, elapsed_time_mst, alter_cost, elapsed_time_alter, cost_dp, elapsed_time_dp, ratio_mst, ratio_alter
...
with open('tsp_results.csv', 'w', newline='') as file:
    writer = csv.writer(file)
    writer.writerow(["Num Countries", "MST Cost", "MST Time (ms)", "MST Alter Cost", "MST Alter Time (ms)", "DP Cost", "DP Time (ms)", "MST Ratio", "MST Alter Ratio"])
